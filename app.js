...
/**
 * 이전 7-3 and 7-4 예제와 차이점
 *
 * app.js 코드에 한해서는 차이점이 전혀 없음.
 *
 * 다만 7-3 and 7-4 예제는
 * 버텍스 셰이더 내에서 각 버텍스들의 밝기값만 계산해준 뒤,
 * varying 변수에 밝기값을 할당하여 프래그먼트 셰이더로 넘기는 '고라우 셰이딩' 방식으로
 * interpolation(보간)을 해줬다면,
 *
 * 7-5 and 7-6 예제는
 * 버텍스 셰이더 내에서는 각 버텍스의 노멀 데이터 및 좌표 데이터만 계산해준 뒤,
 * 눈 좌표계로 변환하고, 그것들을 각각의 varying 변수로 넘겨주면,
 * 프래그먼트 셰이더에서는 각 버텍스들의 노멀, 좌표 데이터를 받아와서
 * 모든 프래그먼트(픽셀)들에 대해서 직접 조명 계산을 해주는 '퐁 셰이딩' 방식으로
 * interpolation(보간)을 해줬다고 보면 됨.
 * -> 이 '퐁 셰이딩'이 더 고품질 결과를 보여주는 기법!
 */
